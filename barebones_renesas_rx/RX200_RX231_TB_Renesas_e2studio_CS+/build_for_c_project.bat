...
if exist ".\%~nx0" (
    set ide=csp
    set pj=.\
    rem the following is CCRX only
    set tc=CCRX
) else if exist "..\%~nx0" (
    set ide=e2
    set pj=..\
    rem the following is GNUC or CCRX
    set tc=CCRX
) else (
    echo\
...
if %ide%==e2 if %tc%==CCRX set TOOLCHAIN_TOOLS_CHECK=make.exe ccrx.exe
if %ide%==e2 if %tc%==GNUC set TOOLCHAIN_TOOLS_CHECK=make.exe rx-elf-gcc.exe
...
if %cpp%==true (
    ...
    if exist "%pj%src\smc_gen\r_bsp\mcu\all\r_typedefs.h" (
        rem RENAME_FILE
        echo Renaming file: "src\smc_gen\r_bsp\mcu\all\r_typedefs.h" --^> "r_typedefs__unused.h"
        move /y "%pj%src\smc_gen\r_bsp\mcu\all\r_typedefs.h" "%pj%src\smc_gen\r_bsp\mcu\all\r_typedefs__unused.h" > nul
        if exist "%pj%src\smc_gen\r_bsp\mcu\all\r_typedefs.h" (
            rem FAILED
            goto ERROR
        )
    ) else if exist "%pj%src\smc_gen\r_bsp\mcu\all\r_typedefs__unused.h" (
        rem ALREADY_RENAME_FILE
        echo Renamed file: "src\smc_gen\r_bsp\mcu\all\r_typedefs.h" --^> "r_typedefs__unused.h"
    )
    echo Use "src\r_bsp_modified\r_typedefs.h" instead of "src\smc_gen\r_bsp\mcu\all\r_typedefs.h"
)
...
echo rx-elf-size "%BUILDARTIFACTFILENAME%"
rx-elf-size "%BUILDARTIFACTFILENAME%"
exit %ERRORLEVEL%
